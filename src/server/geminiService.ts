...
function cleanOfflineSentence(raw: string): string {
  let s = raw.trim();
  if (!s) return "";
...
export async function enhanceSentenceGrammar(rawSentence: string, customApiKey?: string): Promise<string> {
  if (!rawSentence || !rawSentence.trim()) return "";

  const aiClient = getAiClient(customApiKey);
  if (!aiClient) {
    return cleanOfflineSentence(rawSentence);
  }

  const prompt = `You are an expert American Sign Language (ASL) and English linguistic translator.
The user provides a raw sequence of signs, glosses, or broken words assembled by a sign language recognition system (e.g. "HELLO YOU THANK YOU HELP ME" or "ME GO STORE YESTERDAY BUY MILK").

Raw text: "${rawSentence.trim()}"

TASK:
Convert this raw sequence into a single, natural, grammatically correct English sentence.
Rules:
1. Fix broken syntax, grammar, verb tenses, articles, and word order.
2. Restore proper punctuation and capitalization.
3. Keep the original intent, meaning, and tone intact.
4. Output ONLY the polished sentence. Do not include quotes, markdown bolding, notes, or explanations.`;
  ...
  for (const modelName of candidateModels) {
    try {
      ...
      console.warn(`Sentence grammar enhancement with ${modelName} failed:`, err);
    }
  }
...
export async function translateEnglishToSign(englishText: string, customApiKey?: string) {
  if (!englishText || !englishText.trim()) {
    return {
      gloss: "",
      tokens: [],
      instructions: "Enter English text to convert to ASL.",
    };
  }

  const aiClient = getAiClient(customApiKey);
  if (!aiClient) {
    const words = englishText.toUpperCase().replace(/[^A-Z0-9\s]/g, "").split(/\s+/).filter(Boolean);
    return {
      gloss: words.join(" "),
      tokens: words,
      instructions: "Fingerspell or sign each word in visual order.",
    };
  }

  const prompt = `You are a certified master ASL (American Sign Language) linguist and interpreter.
Convert the following English phrase into proper ASL Gloss format with Topic-Comment grammar.

English: "${englishText.trim()}"

Return ONLY a JSON object formatted as follows (no markdown backticks, no code fences):
{
  "gloss": "ASL GLOSS in ALL CAPS (e.g. YESTERDAY STORE ME GO BUY APPLE)",
  "tokens": ["ARRAY", "OF", "ASL", "WORDS"],
  "instructions": "1 concise sentence explaining facial expression or signing space",
  "facialMarker": "Raised eyebrows / Furrowed brow / Neutral / Nod"
}`;
  ...
  for (const modelName of candidateModels) {
    try {
      ...
      return {
        gloss: parsed.gloss || englishText.toUpperCase(),
        tokens: Array.isArray(parsed.tokens) ? parsed.tokens : englishText.toUpperCase().split(/\s+/),
        instructions: parsed.instructions || "Sign in smooth sequence from left to right in your signing space.",
        facialMarker: parsed.facialMarker || "Neutral facial expression",
      };
    } catch (err) {
      console.warn(`English to ASL translation with ${modelName} failed:`, err);
    }
  }
  ...
  return {
    gloss: words.join(" "),
    tokens: words,
    instructions: "Sign clearly within your chest signing zone.",
